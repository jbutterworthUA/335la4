...
public class LibrarySetup {
	...
	/**
	 * Constructor for the LibrarySetup class. Will initialize new masterList and
	 * ratedList objects to store books.
	 */
	public LibrarySetup() {
	...
	/**
	 * Given a txt file from the user, add all books within that file to the
	 * masterList.
	 */
	...
	/**
	 * Retrieve all books in the user's library, organized by user-determined
	 * method. (Title/Author/Read Status)
	 */
		...
		if (sortMethod.equals("read")) {
			...
			for (int i = 0; i < masterList.size(); i++) {
				if (masterList.get(i).isRead()) {
					newMasterList.addBook(masterList.get(i).getBook().getTitle(),
							masterList.get(i).getBook().getAuthor());
				}
			}
	...
	/**
	 * Helper method to retrieve all books which have the same title the user has
	 * inputted.
	 * 
	 * @param searchTitle
	...
	private ArrayList<BookRead> searchByTitle(String searchTitle) {
		...
		// Search every book in masterList, checking if the title matches our
		// searchTitle.
		for (int i = 0; i < masterList.size(); i++) {
	...
	/**
	 * Helper method to retrieve all books which have the same author the user has
	 * inputted.
	 * 
	 * @param searchAuthor
	 * @return ArrayList of all possible books the user could be searching for.
	...
	/**
	 * Helper method to retrieve all books which have the same rate the user has
	 * inputted.
	 * 
	 * @param searchRate
	 * @return ArrayList of all possible books the user could be searching for.
	...
	public boolean rate(String searchTitle, String searchAuthor, int newRate) {
		...
		// Check if the book we are searching for is already in ratedList for us to
		// update.
		for (int i = 0; i < ratedList.size(); i++) {
		...
		// If we did not find the book in ratedList, we will search masterList and add
		// it to ratedList.
		for (int i = 0; i < masterList.size(); i++) {
			...
			// When we find the book in masterList, we will need to create BookReview object
			// and add to ratedList.
			if (currentTitle.equals(searchTitle) && currentAuthor.equals(searchAuthor)) {
